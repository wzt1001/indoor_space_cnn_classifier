...
parser.add_argument('--split',default='/media/ztwang/ssd_0/data/indoor_space_cnn_classifier/data/penn_station/datasplit/new_224.npz')
# parser.add_argument('--data', default='/input', metavar='DIR',
#                     help='path to dataset')
parser.add_argument('--outf', default='output',
                    help='folder to output model checkpoints')
parser.add_argument('--nClasses', type=int, default=240)
...
parser.add_argument('-b', '--batchSz', default=48, type=int,
...
def reload_data(path):
    # load indoor datasplit
    data_indoor = np.load(path)
    data_train = data_indoor['data_train'].astype(np.float32)
    # data_train = data_indoor['data_train'].astype(np.float32)
    data_train = data_train.reshape((data_train.shape[0], 3, 224, 224))
    print(data_train.shape)
    data_train = torch.from_numpy(data_train)
    label_train = data_indoor['label_train']
    label_train = torch.from_numpy(label_train.astype(int))

    print ('loaded indoor image dataset')
...
def main():
    ...
    print ('loaded indoor image validation dataset')
    val_d = TensorDataset(data_val, label_val)
    val_loader = DataLoader(val_d, batch_size=args.batchSz, shuffle=False)

    train_loader = reload_data("/home/ztwang/ssd_data/data/indoor_space_cnn_classifier/data/penn_station/datasplit/274_split_244_train2.npy.npz")
    labels = 240
    ...
    if args.train:
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            # if epoch == 100:
            #     train_loader = reload_data("/home/ztwang/ssd_data/data/indoor_space_cnn_classifier/data/penn_station/datasplit/274_split_244_train1.npy.npz")
            # elif epoch == 200:
            #     train_loader = reload_data("/home/ztwang/ssd_data/data/indoor_space_cnn_classifier/data/penn_station/datasplit/274_split_244_train2.npy.npz")
